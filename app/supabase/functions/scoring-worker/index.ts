...
} from "../_shared/scoring-explanation.ts";

const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const WORKER_SECRET = Deno.env.get("SCORING_WORKER_SECRET")!;
...
// else ever revisits it. reclaim_stale_visibility_explanations() (migration 0016)
// resets anything still 'processing' after this long back to 'retry'/'failed'.
// Kept comfortably above this worker's realistic per-batch runtime so a
// genuinely in-flight job is never reclaimed out from under itself.
...
type Job = {
  ...
  // Precomputed by run_visibility_scoring_cycle() and passed straight through
  // by claim_visibility_explanation_jobs() -- never recompute this via
  // division here, see buildExplanationPrompt's design note.
  citation_ratio: number;
  brand_citation_profile: { domain_type: string; pct: number }[];
  competitor_citation_profile: { domain_type: string; pct: number }[];
  opportunity_gaps: {
    domain_type: string;
...
async function rpc<T>(name: string, body: Record<string, unknown>): Promise<T> {
  ...
  // (retry_or_fail_visibility_explanation, complete_visibility_explanation) unless
  // a `Prefer: return=representation` header is sent, which this helper doesn't
  // send. `.json()` on an empty body throws "Unexpected end of JSON input" --
...
async function getFailoverMode(provider: "gemini" | "nvidia_nim"): Promise<FailoverMode> {
  const rows = await fetch(
    `${SUPABASE_URL}/rest/v1/ai_provider_settings?provider=eq.${provider}&select=failover_mode`,
    { headers: headers() },
  ).then((r) => r.json() as Promise<Array<{ failover_mode: FailoverMode }>>);
  return rows[0]?.failover_mode ?? "shared";
}

async function getKeyHealth(provider: "gemini" | "nvidia_nim"): Promise<Set<KeySlot>> {
  const health = await fetch(
    `${SUPABASE_URL}/rest/v1/ai_provider_key_health?provider=eq.${provider}&select=key_slot,is_dead`,
    { headers: headers() },
  ).then((r) => r.json() as Promise<Array<{ key_slot: KeySlot; is_dead: boolean }>>);
...
async function processJobWithStagger(job: Job, index: number): Promise<void> {
  ...
  try {
    ...
    const task = await resolveTaskModel("explanation_generation", job.workspace_id);

    if (task.provider !== "nvidia_nim") {
      throw new Error(
        `explanation_generation task resolved to unsupported provider '${task.provider}' -- only nvidia_nim is implemented`,
      );
    }
    ...
    const prompt = buildExplanationPrompt({
      ...
      competitorName: job.competitor_name,
      brandMentionCount: job.brand_mention_count,
      competitorMentionCount: job.competitor_mention_count,
      citationRatio: job.citation_ratio,
      brandCitationProfile: job.brand_citation_profile,
      competitorCitationProfile: job.competitor_citation_profile,
      opportunityGaps: job.opportunity_gaps,
    });
    ...
    // Parse and validate the explanation response
    const result: ExplanationResult = parseExplanationResponse(rawText);

    // Record successful result
    await rpc("complete_visibility_explanation", {
      p_snapshot_id: job.snapshot_id,
      p_provider: task.provider,
      p_model: task.model,
      p_explanation_text: result.explanation_text,
      p_recommended_actions: result.recommended_actions,
    });

    log("info", "Explanation job completed successfully", {
    ...
    await rpc("retry_or_fail_visibility_explanation", {
      p_snapshot_id: job.snapshot_id,
      p_error_code: errorCode,
    });

    log("error", "Explanation job failed", {
      snapshotId: job.snapshot_id,
      errorCode,
      retryAfter,
...
Deno.serve(async (request) => {
  const providedSecret = request.headers.get("x-scoring-worker-secret") ?? "";
  if (
    request.method !== "POST" ||
    !WORKER_SECRET ||
  ...
  try {
    ...
    const reclaimed = await rpc<number>("reclaim_stale_visibility_explanations", {
      p_stale_after_minutes: STALE_JOB_MINUTES,
    });
    if (reclaimed > 0) log("warn", "Reclaimed stale explanations", { requestId, reclaimed });

    // Claim up to 10 jobs for processing
    const jobs = await rpc<Job[]>("claim_visibility_explanation_jobs", { p_limit: 10 });

    log("info", "Explanation jobs claimed", { requestId, count: jobs.length });
