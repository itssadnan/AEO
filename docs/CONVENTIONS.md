...
**Exception — Client Components and barrel files.** Turbopack/webpack's Client/Server Component boundary check runs on the *static import graph* of a barrel file (`export { x } from "./y"`), not on which named exports are actually used. If `index.ts` re-exports anything from a file that imports server-only code (e.g. `@/lib/db`'s `createSupabaseServerClient`, which pulls in `next/headers`), then **any** `"use client"` file that imports the barrel — even for a completely unrelated, pure, dependency-free export — pulls that server-only module into the client bundle and fails the production build. This was confirmed directly against real Vercel build error traces during Module 5.7 (see `progress/modules/5.7-crawl-readiness-audit.md` decisions log) — splitting the pure function into its own file was not sufficient on its own, because the barrel's re-export statement still statically imports the server-touching file.

The fix, when a module needs to expose something to a Client Component: put the client-safe code in its own file with **zero imports from anywhere that touches `@/lib/db`, `next/headers`, or `server-only`** (types-only files already qualify), keep it re-exported from `index.ts` for every other consumer as normal, and let the `"use client"` file import that specific file directly (e.g. `@/modules/<name>/<pure-file>`), bypassing the barrel only for that import. Document the exception with a comment at both ends (the pure file and the client component) pointing back to this section. This is the one narrow, deliberate deviation from the barrel-only rule — it exists because of a bundler limitation, not because encapsulation stopped mattering.
