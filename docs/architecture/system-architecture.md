...
- **5.9 Billing & Subscription** — gates 5.2 (prompt-count limits via a DB trigger on `prompts`), 5.2 (brand-count limits via a DB trigger on `brands`), and 5.3 (check frequency via `check_jobs` scheduling) based on Razorpay subscription state. Reads/writes `subscriptions`; plan-tier enforcement is trigger-based, not a separate counter table (see the entity-relationship diagram's design decisions).
- **5.10 Admin/Ops Console** — internal view for platform health and operations: per-key-slot API quota consumption (primary/secondary/tertiary) for Gemini and NVIDIA NIM, key-pool health (alive/dead status with manual clear), failover mode toggle (`shared` vs `emergency-only`), error/failure log for failed checks, basic churn signals (customers inactive 14+ days), and a full AI task-config editor (global defaults + per-workspace overrides for `ai_task_configs`). Reads from `ai_provider_key_health`, `ai_provider_settings`, `engine_error_logs`, `check_runs`, `check_jobs`, `workspace_members`, and `ai_task_configs`. Access is gated by a `platform_admins` allowlist checked server-side via `requireAdmin()` on top of the same Supabase Auth session customers use, not a separate admin login (see 5.10's decisions log).
- **5.7 Crawl-Readiness Audit** — runs independently against the _customer's own site_ (not an AI engine), on its own schedule, and writes into the same `crawl_audits` table the Dashboard (5.6) reads from. No dependency on 5.3-5.5.
